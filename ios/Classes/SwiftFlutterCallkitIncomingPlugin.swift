...
public class SwiftFlutterCallkitIncomingPlugin: NSObject, FlutterPlugin, CXProviderDelegate {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "callConnected":
            ...
            break
        case "activeCalls":
            let activeCalls = self.callManager.activeCalls()
            let acceptedCall = answerCall?.uuid.uuidString as? String ?? ""
            var activeCallsWithAccepted = self.callManager.activeCalls().map { call in
                var updatedCall = call
                updatedCall["accepted"] = (call["uuid"] as? String)?.lowercased() == acceptedCall.lowercased()
                return updatedCall
            }
            result(activeCallsWithAccepted)
            break;
        case "endAllCalls":
...
//                self.endCallNotExist(data)
    ...
    public func provider(_ provider: CXProvider, perform action: CXSetGroupCallAction) {
        print("call? CXSetGroupCallAction")
        print(self.callManager.calls)
